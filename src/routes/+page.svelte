<script>
  import { base } from "$app/paths";
</script>
<div
  class="hero min-h-screen"
  style="background-image: url(/alkendi-academy/images/hero-image.jpg);">
  <div class="hero-overlay bg-opacity-90"></div>
  <div class="hero-content text-neutral-content text-center">
    <div class="max-w-mx">
      <h1 class="mb-5 text-5xl text-white font-bold">We are waiting for You</h1>
      <p class="mb-5 text-white py-6 text-xl md:text-3xl lg:text-4xl font-semibold bg-white/20 p-2 rounded-md">
        Whether you want to make small improvements to your lifestyle or treat yourself to some useful knowledge, the BTS community can help you start something new this year.
      </p>
      <a href="{base}/events" class="btn btn-primary">Discover our Events</a>
    </div>
  </div>
</div>
